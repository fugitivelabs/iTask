...
exports.listByValues = (req, res) => {
  ...
  if (!req.query[req.params.refKey]) {
    ...
    res.send({
      success: false,
      message: `Missing query param(s) specified by the ref: ${req.params.refKey}`,
    });
  } else {
    Note.find(
      { [req.params.refKey]: { $in: [].concat(req.query[req.params.refKey]) } },
      (err, notes) => {
        if (err || !notes) {
          res.send({
            success: false,
            message: `Error querying for notes by ${[req.params.refKey]} list`,
            err,
          });
        } else {
  ...
  if (nextParams.split("/").length % 2 == 0) {
    ...
    if (nextParams.length !== 0) {
      for (let i = 1; i < nextParams.split("/").length; i += 2) {
        query[nextParams.split("/")[i]] =
          nextParams.split("/")[i + 1] === "null"
            ? null
            : nextParams.split("/")[i + 1];
      }
    }
    Note.find(query, (err, notes) => {
      if (err || !notes) {
        res.send({
          success: false,
          message: `Error retrieving notes by ${req.params.refKey}: ${req.params.refId}`,
        });
      } else {
        res.send({ success: true, notes });
      }
...
exports.create = (req, res) => {
  ...
  note
    ...
    .then((n) => {
      n.populate("_user", "firstName lastName")
        .execPopulate()
        .then((note) => {
          res.send({ success: true, note: note });
        });
